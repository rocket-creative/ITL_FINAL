...
function viabilityNote(gene: GeneRow): string {
  switch (gene.impc_viability) {
    case 'lethal':
      return `Published IMPC data show constitutive null alleles of ${gene.symbol} are embryonic lethal. A floxed conditional allele lets you delete later in a chosen tissue without losing the line to global loss.`;
    case 'subviable':
      return `Homozygous ${gene.symbol} null animals show reduced viability in IMPC summaries. Conditional or inducible designs help you remove function after development finishes.`;
    case 'viable':
      return `Published viability data support viable ${gene.symbol} null or hypomorphic alleles. Constitutive and conditional routes are both on the table depending on your readout.`;
    default:
      return `${gene.symbol} is a well studied locus in mouse genetics. Allele design follows your phenotype goals and what the published landscape already shows for this gene.`;
  }
}
...
function modificationRationale(modelType: ModelTypeRow): string {
  ...
  if (slug === 'conditional-knockout' || slug === 'inducible-knockout') {
    return `Conditional alleles place loxP sites around critical exons and keep reading frame intact until Cre excision. FRT flanked derivatives (tm1a to tm1c to tm1d) let you convert before breeding to your Cre driver. We confirm critical exon choice and regulatory element preservation before germline transmission.`;
  }
  if (slug === 'knockout') {
    return `Knockout alleles remove or disrupt coding sequence using gene targeting. Critical exon selection and frame considerations are confirmed before microinjection so founders carry a defined null or hypomorphic allele.`;
  }
  if (slug === 'knockin' || slug === 'cdna-knockin' || slug === 'tag-knockin') {
    return `Knockin alleles insert sequence at the endogenous locus under native regulation. Homologous recombination targeting confirms junction integrity and expression pattern before germline transmission, which keeps physiological control cleaner than random integration.`;
  }
  if (slug === 'point-mutation') {
    return `Point mutation knockin recreates patient relevant nucleotide changes at the endogenous locus. Homology arms and silent restriction sites are engineered so the variant is expressed from the native promoter, with sequence confirmation before germline transmission.`;
  }
  if (slug === 'humanized') {
    return `Humanization replaces mouse coding sequence with human ortholog sequence at the endogenous locus while keeping native regulatory context. Gene targeting documents junctions and expression before germline transmission.`;
  }
  if (slug === 'reporter') {
    return `Reporter alleles fuse a detectable marker to endogenous regulation so expression mirrors the native locus. Pre germline characterization confirms signal location before germline transmission.`;
  }
  if (slug === 'overexpression') {
    return `Targeted overexpression places your transgene at a safe harbor or Rosa26 locus with defined copy number and expression control. That avoids random integration position effects caught during pre germline screening.`;
  }
  if (slug === 'cre-driver') {
    return `Cre driver lines express Cre recombinase from ${modelType.display_name.toLowerCase()} regulatory elements. Promoter choice follows published expression data so recombination lines up with your tissue or cell type of interest.`;
  }
  return `Allele design follows published structure for this locus, with homologous recombination targeting and pre germline characterization.`;
}
...
function typeSpecificNote(gene: GeneRow, modelType: ModelTypeRow): string {
  ...
  if (slug === 'cre-driver' && gene.expression_profile) {
    ...
    return `Expression data support ${gene.symbol} activity in ${tissues}, which fits a promoter driven Cre driver for tissue restricted recombination.`;
  }
  if (slug === 'inducible-knockout') {
    return `Inducible systems (CreERT2, tamoxifen, or Tet regulated) pair with a floxed ${gene.symbol} allele so loss of function can be timed after development.`;
  }
  return '';
}
...
export function buildPageCopy(gene: GeneRow, modelType: ModelTypeRow): PageCopy {
  ...
  const timeline =
    'Quotes return in about 24 hours with milestones, pricing, and options for cryo or live dispatch.';
