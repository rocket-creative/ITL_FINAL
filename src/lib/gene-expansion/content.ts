...
function modificationRationale(modelType: ModelTypeRow): string {
  ...
  if (slug === 'conditional-knockout' || slug === 'inducible-knockout') {
    return `Conditional alleles place loxP sites around critical exons while preserving reading frame until Cre recombinase excision. FRT flanked derivative alleles (tm1a to tm1c to tm1d) allow flexible conversion before breeding to your Cre driver. Critical exon choice and regulatory element preservation are validated prior to germline transmission.`;
  }
  if (slug === 'knockout') {
    return `Knockout alleles remove or disrupt coding sequence at the ${modelType.display_name.toLowerCase()} locus using gene targeting. Critical exon selection and frame considerations are confirmed before microinjection so founders carry a defined null or hypomorphic allele.`;
  }
  if (slug === 'knockin' || slug === 'cdna-knockin' || slug === 'tag-knockin') {
    return `Knockin alleles insert sequence at the endogenous locus under native regulation. Homologous recombination targeting confirms junction integrity and expression pattern before germline transmission, preserving physiological control compared with random integration.`;
  }
  ...
  if (slug === 'humanized') {
    return `Humanization replaces mouse coding sequence with human ortholog sequence at the endogenous locus, maintaining native regulatory context. Gene targeting documents junctions and expression before germline transmission.`;
  }
  if (slug === 'reporter') {
  ...
  if (slug === 'overexpression') {
    return `Targeted overexpression places your transgene at a safe harbor or Rosa26 locus with defined copy number and expression control, avoiding random integration position effects documented during pre germline screening.`;
  }
  if (slug === 'cre-driver') {
...
export function buildPageCopy(gene: GeneRow, modelType: ModelTypeRow): PageCopy {
  ...
  const geneFraming = `${gene.symbol}${gene.name ? ` (${gene.name})` : ''} is a focus locus for ${modelType.display_name.toLowerCase()} mouse models. ingenious targeting laboratory designs and delivers the line to your study specification with documented milestones.`;
